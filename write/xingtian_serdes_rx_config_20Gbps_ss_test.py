...
### Firstly, rx is power down & reset.
...
### rx_bias_pd is controlled by rx_pd.
write_spi_reg(0x00,0x33,0X0)

write_spi_reg(0x00,0x02,0x20)  ### According to measurement result, term register should be set to 0.
write_spi_reg(0x00,0x04,0x7F)
write_spi_reg(0x00,0x05,0x88)
write_spi_reg(0x00,0x06,0x1)
write_spi_reg(0x00,0x07,0xE)
write_spi_reg(0x00,0x08,0x0)

### Execute slicer calibration.
### slicer calibration reset_n=1
write_spi_reg(0x00,0x30,0X82)
...
write_spi_reg(0x00,0xA7,0x20)  ### According to measurement result, term register should be set to 0.
...
write_spi_reg(0x00,0xD5,0X82)

write_spi_reg(0x00,0xD6,0X60)
...
print("addr(0x219)=%02x\n" % (read_buffer[0]))
read_buffer = read_spi_reg(0x82,0x1A)
print("addr(0x21A)=%02x\n" % (read_buffer[0]))
read_buffer = read_spi_reg(0x82,0x1B)
print("addr(0x21B)=%02x\n" % (read_buffer[0]))
read_buffer = read_spi_reg(0x82,0x1C)
print("addr(0x21C)=%02x\n" % (read_buffer[0]))

sleep(0.2)
### Error counter insert error
write_spi_reg(0x00,0xDC,0x34)
sleep(0.2)
### Read error counter value again
write_spi_reg(0x00,0xDC,0x36)
sleep(0.2)
read_buffer = read_spi_reg(0x82,0x19)
print("addr(0x219)=%02x\n" % (read_buffer[0]))
read_buffer = read_spi_reg(0x82,0x1A)
