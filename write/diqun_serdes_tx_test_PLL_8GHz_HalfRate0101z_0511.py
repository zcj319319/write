...
write_spi_reg(0x11,0x48,0x40) ####div  by 24
##write_spi_reg(0x11,0x48,0x3c) ####div  by 60
##write_spi_reg(0x11,0x48,0x40) ####div by 64

####PLL settings
write_spi_reg(0x11,0x3a,0x90) ####AMP_CTRL[1:0],[5] Reserved,BY_SLLP,BY_Freq,BY_AMP,BY_PLL,SHORT_RC
write_spi_reg(0x11,0x3b,0xfd) ####AMP_CALI_DIVR[1:0],AMPD_SEL[2:0],AMP_CTRL[2:0]
write_spi_reg(0x11,0x3c,0x07) #### FREQ_LOAD[[5:0],AMP_CALI_DIVR[3:2]  #0x02 by default.
write_spi_reg(0x11,0x3d,0x05) #### FREQ_LOAD[13:6]
write_spi_reg(0x11,0x3e,0x90) #### FREQ_CALI_CNT2_SIZE[2:0],CNT1_SIZE[2:0],FREQ_LOAD[15:14]
write_spi_reg(0x11,0x3f,0x81) #### FREQ_CALI_STEP2_SIZE[0],STEP1_SIZE[6:0]
write_spi_reg(0x11,0x40,0x00) #### TEMP_BY_SLLP[1:0],FREQ_CALI_STEP2_SIZE[6:1]
write_spi_reg(0x11,0x41,0x0f) #### TEMP_BY_SLLP[9:2]
write_spi_reg(0x11,0x42,0x02) #### TEMP_SLLP[4:0], TEMP_CLK_SEL,TEMP_BY_SLLP[11:10]
write_spi_reg(0x11,0x43,0x40) #### SLLP_CLK_DIV[0],TEMP_SLLP[11:5]
write_spi_reg(0x11,0x44,0xf0) #### SLLP_CLK_DIV[8:1]
write_spi_reg(0x11,0x45,0x57) #### IND_SW_MODE, IND SW, SLLP_STEP[2:0],SLLP_CLK_DIV[11:9]
write_spi_reg(0x11,0x46,0x2f) #### REF_DIVR[3:0],KVCO_SLOW,KVCO[2:0]
write_spi_reg(0x11,0x49,0x00) #### LPF Res[0]=0,LPF_C2[2:0]=4，FBC_DIVR[11:8]
write_spi_reg(0x11,0x4a,0x50) #### Enable post divider. ICP[2:0]=2,LPF_Res[1]=0,
#write_spi_reg(0x11,0x4a,0x56) #### Enable post divider. ICP[2:0]=3,LPF_Res[1]=0,
write_spi_reg(0x11,0x50,0x49) ### VSEL_VREF_CMP[0],VSEL_VAR_FASR[2:0],VSEL_VAR_BS[2:0=,VSEL_SW_CAP_BS[0],0x49 by default
write_spi_reg(0x11,0x51,0x16) ### [7:5]reserved,VSEL_VREF_LPF[2:0],VSEL_VREF_CMP[2:1],0x12 defualt
### High temperature start up, load 0x11,0x51,0x1a.
### Low temperature start up, load 0x11,0x51,0x12.
